...
export class TranscriptView {
  ...
  constructor(container: HTMLElement) {
    ...
    this.container.addEventListener("pointerdown", this._handlePointerDown, { passive: true });
    this.container.addEventListener("pointerup", this._handlePointerUp, { passive: true });
    ...
    switch (item.kind) {
      case "message": {
        ...
        if (item.role === "user") {
          ...
          el.innerHTML = `
            <div class="relative max-w-[80%] bg-primary text-white rounded-2xl rounded-tr-sm px-4 py-3">
          ...
          el.innerHTML = `
            <div class="flex-shrink-0 w-8 h-8 rounded-full bg-surfaceContainerHighest flex items-center justify-center text-primary self-end mb-2">
              <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
            </div>
            <div class="relative max-w-[85%] bg-surfaceContainer rounded-2xl rounded-bl-sm px-4 py-3">
      ...
      case "tool_call": {
        ...
        el.innerHTML = `
           <div class="tool-card bg-surfaceContainer rounded-xl overflow-hidden transition-all duration-200">
             <!-- Header -->
             <div class="tool-card-header bg-surfaceVariant px-3 py-2 flex items-center justify-between">
                <div class="flex items-center gap-2">
                    <div class="status-indicator w-1.5 h-1.5 rounded-full ${isComplete ? (isError ? 'bg-danger' : 'bg-success') : 'bg-info animate-pulse'}"></div>
             ...
             <div class="tool-result-container hidden bg-surfaceVariant p-2 text-xs">
